...
Scaffold Chrome Extension Manifest V3 untuk menambahkan bulk download PDF pada beberapa halaman Coretax.

## Isi scaffold

- `manifest.json`: registrasi content script untuk domain Coretax
- `content.js`: inject toolbar bulk action, sinkronisasi selection, dan antrean klik tombol PDF
- `content.css`: styling toolbar, checkbox, dan highlight baris

## Halaman yang didukung

- `returnsheets-portal/.../submitted-returnsheets`
  Menggunakan checkbox yang diinjeksi extension.
- `e-invoice-portal/.../output-tax`
  Menggunakan checkbox bawaan grid Coretax.
...
- Untuk halaman submitted return sheets:
  - Menambahkan checkbox di kolom aksi tiap baris
  - Menambahkan checkbox `Bulk` pada header tabel
- Untuk halaman output tax:
  - Membaca selection dari checkbox bawaan Coretax
  - Men-trigger tombol `DownloadButton` per baris yang dipilih
...
6. Buka halaman Coretax `submitted-returnsheets` atau `output-tax`.
